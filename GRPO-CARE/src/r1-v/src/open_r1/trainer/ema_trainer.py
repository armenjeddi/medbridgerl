...
def moving_average(model, model_ema, accelerator, beta=0.992, device=None, zero_stage=0):
    ...
    if beta == 0:
        clone_zero_model(src_model=model, dst_model=model_ema, zero_stage=zero_stage)
    else:
...
class EmaTrainer:
    ...
    # def _get_second_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
    #     if self.second_train_dataset is None or not has_length(self.second_train_dataset):
    #         return None

    #     # Build the sampler.
    #     if self.args.group_by_length:
    #         if is_datasets_available() and isinstance(self.second_train_dataset, datasets.Dataset):
    #             lengths = (
    #                 self.second_train_dataset[self.args.length_column_name]
    #                 if self.args.length_column_name in self.second_train_dataset.column_names
    #                 else None
    #             )
    #         else:
    #             lengths = None
    ...
    #         return LengthGroupedSampler(
    #             self.args.train_batch_size * self.args.gradient_accumulation_steps,
    #             dataset=self.second_train_dataset,
    #             lengths=lengths,
    #             model_input_name=model_input_name,
    #         )

    #     else:
    #         return RandomSampler(self.second_train_dataset)

    # def get_second_train_dataloader(self) -> DataLoader:
    #     """
    #     Returns the training [`~torch.utils.data.DataLoader`].

    #     Will use no sampler if `second_train_dataset` does not implement `__len__`, a random sampler (adapted to distributed
    #     training if necessary) otherwise.

    #     Subclass and override this method if you want to inject some custom behavior.
    #     """
    #     if self.second_train_dataset is None:
    #         raise ValueError("Trainer: training requires a second_train_dataset.")

    #     second_train_dataset = self.second_train_dataset
    #     data_collator = self.data_collator
    #     if is_datasets_available() and isinstance(second_train_dataset, datasets.Dataset):
    #         second_train_dataset = self._remove_unused_columns(second_train_dataset, description="training")
    #     else:
    #         data_collator = self._get_collator_with_removed_columns(data_collator, description="training")
    ...
    #     if not isinstance(second_train_dataset, torch.utils.data.IterableDataset):
    #         dataloader_params["sampler"] = self._get_second_train_sampler()
    #         dataloader_params["drop_last"] = self.args.dataloader_drop_last
    #         dataloader_params["worker_init_fn"] = seed_worker
    #         dataloader_params["prefetch_factor"] = self.args.dataloader_prefetch_factor

    #     return self.accelerator.prepare(DataLoader(second_train_dataset, **dataloader_params))
    ...
    def _inner_training_loop(
        ...
        #     second_train_dataloader = self.get_second_train_dataloader()
        #     if self.is_fsdp_xla_v2_enabled:
        #         second_train_dataloader = tpu_spmd_dataloader(second_train_dataloader)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            # if self.second_train_dataset is not None:
            #     second_epoch_dataloader = second_train_dataloader
            #     if hasattr(second_epoch_dataloader, "set_epoch"):
            #         second_epoch_dataloader.set_epoch(epoch)
            ...
            # if self.second_train_dataset is not None:
            #     steps_in_epoch = steps_in_epoch * 2
            ...
            # if self.second_train_dataset is not None:
            #     second_epoch_iterator = iter(second_epoch_dataloader)
            ...
            for _ in range(total_updates):
                ...
                #         # print(batch_samples)
                #     else:
                #         batch_samples, num_items_in_batch = self.get_batch_samples(second_epoch_iterator, num_batches, args.device)
                #         # print(batch_samples)
                # else:
                batch_samples, num_items_in_batch = self.get_batch_samples(epoch_iterator, num_batches, args.device)
                # print(batch_samples)
